...
void DyScaleFactors::produceScaleFactors(const Samples& samples)
{
    ...
    const std::vector<std::pair<TString, TString> > v_nameStepPair = utils::nameStepPairs(filename, "dyScaling_Allh1_step");
                                     ...
                                     const Systematic::Systematic& systematic)const
{
    ...
    const TString step = utils::extractSelectionStep(fullStepname);
    if(step == ""){
    ...
    std::cout << fullStepname <<  step <<std::endl;
