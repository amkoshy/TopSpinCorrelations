...
AnalyzerKinReco::AnalyzerKinReco(const Era::Era era,
...
void AnalyzerKinReco::bookHistos(const TString& step, std::map<TString, TH1*>& m_histogram)
{
	...
	name = "pTt_true_vs_reco_pTt300";
        ...
        name = "epsilon";
        m_histogram[name] = store(new TH1D ( prefix_+name+step, "epsilon", 10000, 0, 0.00000004 ));

}



void AnalyzerKinReco::fillHistos(const EventMetadata& ,
    ...
    if(recoObjects.valuesSet_){
        ...
        for(const int reco_j : (recoObjectIndices.bjetIndices_))
        {
            ...
            for(int gen_j=0;gen_j<(int)(*topGenObjects.allGenJets_).size();gen_j++)
            {
                ...
                if(era_ == Era::run1_8tev) {
                ...
                else if(era_ == Era::run2_13tev_50ns || era_ == Era::run2_13tev_25ns || era_ == Era::run2_13tev_2016_25ns || era_ == Era::run2_13tev_2016 || era_ == Era::run2_13tev_2017 || era_ == Era::run2_13tev_2018 || era_ == Era::run2_UL_13tev_2016preVFP || era_ == Era::run2_UL_13tev_2016postVFP || era_ == Era::run2_UL_13tev_2017 || era_ == Era::run2_UL_13tev_2018) {
                    double e = 0.000000001;
                    double dX = fabs(genjet.Px()-asedjet.Px())/fabs(genjet.Px());
                    double dY = fabs(genjet.Py()-asedjet.Py())/fabs(genjet.Py());
                    double dZ = fabs(genjet.Pz()-asedjet.Pz())/fabs(genjet.Pz());
                    double dE = fabs(genjet.E()-asedjet.E())/fabs(genjet.E());
                    m_histogram["epsilon"]->Fill(dX*dY*dZ*dE);
                    if(dX*dY*dZ*dE<e) {
                        for (int IDjb=0; IDjb<(int)(*topGenObjects.genBHadJetIndex_).size(); IDjb++)
                        {
                            if(gen_j==(*topGenObjects.genBHadJetIndex_).at(IDjb))
                    ...
                    std::cerr<<"ERROR in AnalyzerKinReco::fillHistos() ! Era is not supported: "
                     <<Era::convert(era_)<<"\n...break\n"<<std::endl;
                    exit(322);
                }
            }
