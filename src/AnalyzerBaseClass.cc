...
void AnalyzerBaseClass::fill(const EventMetadata& eventMetadata,
			     ...
			     const double& weight, const double& weightLooseKinReco, const TString& stepShort)
{
    ...
    this->fillHistos(eventMetadata, recoObjects, commonGenObjects,
		     ...
		     looseKinematicReconstructionSolution,
                     ...
                     weight, weightLooseKinReco, step,
                     m_histogram);
}
...
{
    const TString looseStep = utils::stepName(looseStep_) + "zWindow";
    if(step == looseStep){
...
{
    histo = this->store(new TH1D(name,"Dilepton Mass; m_{ll}; events",80,1,401));
    return histo;
}
