...
void TreeHandlerBase::book()
{
    ...
    for(const auto& stepShort : selectionSteps_){
        const TString step = utils::stepName(stepShort);
        this->addStep(step);
    }
}
...
void TreeHandlerBase::addStep(const TString& step)
{
    ...
    //+++++++++++++++++++++++++++++++++++++++++++++++++++// 
    //ajeeta -- Check that output TFile already exists
    if(!fTreeOutput->IsOpen()) {
        std::cout<<"Set Tree Output file first !!"<<std::endl;
        exit(0);
    }
    ...
    //+++++++++++++++++++++++++++++++++++++++++++++++++++// 
    //ajeeta: skip adding the tree to the TSelectorList so that the TTree remains linked with the TFile directory
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++// 
//ajeeta
void TreeHandlerBase::setOutputFile(const TString& outputFilename,
                                    const Channel::Channel& channel, const Systematic::Systematic& systematic)
{
    ...
    fTreeOutput=new TFile(f_savename, "RECREATE");
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++// 
//ajeeta
void TreeHandlerBase::writeTrees(TH1* hist)
{
...
void TreeHandlerBase::createBranch(TTree* tree, const VariableFloatVector& variable)const
{

    std::string name(variable.name());
    tree->Branch(name.c_str(), const_cast<std::vector<Float_t>*>(&variable.value_));
}
...
void TreeHandlerBase::importTrees(const TString& f_savename, const TString& prefix)
{
    ...
    const std::vector<std::pair<TString, TString> > v_nameStepPair =
        utils::nameStepPairs(f_savename, treeName);
...
/*void TreeHandlerBase::importBranch(TTree* tree, VariableFloatVector& variable)const
{
    tree->SetBranchAddress(variable.name().data(), &variable.value_, &variable.branch_);
}*/
