...
void LuminosityScaleFactors::produceScaleFactors(const Samples& samples, const double& luminosityInInversePb, const double& luminosityUncertaintyRelative)
{
    ...
    for(const auto& systematicChannelSamples : samples.getSystematicChannelSamples()){
        ...
        for(const auto& channelSamples : systematicChannelSamples.second){
            ...
            for(const Sample& sample : samples){
                double weight(-999.);
                if(sample.sampleType() != Sample::data) weight = this->luminosityWeight(sample, luminosityInInversePb*factor);
