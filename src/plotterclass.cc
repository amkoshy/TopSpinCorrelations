...
double Plotter::CalcXSec(std::vector<TString> datasetVec, double InclusiveXsectionVec[4],double InclusiveXsectionStatErrorVec[4], TString Systematic, TString Shift){
   ...
   //    double BranchingFraction[4]={0.01582, 0.01573, 0.03155, 0.06310};//[ee, mumu, emu, combined] including tau into electron/muon - update needed !!!
    double BranchingFraction[4]={0.01147,0.01130,0.02277,0.04554};//[ee, mumu, emu, combined] not including tau
...
//         if(vec_systematic.at(Syst) == "POWHEG") {vec_systematic.at(Syst) = "HAD_";}
    ...
    for(int systs =0; systs<(int)vec_systematic.size(); systs++){
        ...
        //const double AvgSyst = TotalSyst/(bins-2);
        //const double SqAvgSys = TMath::Sqrt(TotalSqSyst/(bins-2));
        fprintf(systfile, "\\\\ \n");
