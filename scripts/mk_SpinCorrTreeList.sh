...
mkdir -p TUnfoldFileLists
rm TUnfoldFileLists/TUnfoldFileList*
...
foreach channel (ee emu mumu)
  foreach Syst  (
    ...
    MATCH_UP MATCH_DOWN  \
    POWHEGV2HERWIG AMCATNLOFXFX
)

    if [ -d /mnt/hadoop/store/group/local/cmstop/jthiema/2016_old/spinCorrInputSymmetrized_2016/$Syst/$channel ] ; then
#        ls -1 /mnt/hadoop/store/group/local/cmstop/ajeeta/spinCorrInput/$Syst/$channel/${channel}_$sample*.root | grep -v _amcatnlofxfx >> TUnfoldFileLists/TUnfoldFileList_$Syst\_$channel.txt
#        ls -1 /mnt/hadoop/store/group/local/cmstop/ajeeta/spinCorrInput/$Syst/$channel/${channel}_$sample*.root | grep -v _amcatnlofxfx >> TUnfoldFileLists/TUnfoldFileList_$Syst\_combined.txt
        ls -1 /mnt/hadoop/store/group/local/cmstop/jthiema/2016_old/spinCorrInputSymmetrized_2016/$Syst/$channel/${channel}_ttbarsignal*.root >> TUnfoldFileLists/TUnfoldFileList_$Syst\_$channel.txt
        ls -1 /mnt/hadoop/store/group/local/cmstop/jthiema/2016_old/spinCorrInputSymmetrized_2016/$Syst/$channel/${channel}_ttbarsignal*.root >> TUnfoldFileLists/TUnfoldFileList_$Syst\_combined.txt
    fi

  end

  foreach sample (run smu_run se_run dyee dymumu dytautau ww wz zz wtolnu ttbarW ttbarZ single ttbarbg ttbarsignal dy1050 dy50inf)
    if [ -d /mnt/hadoop/store/group/local/cmstop/jthiema/2016_old/spinCorrInputSymmetrized_2016/Nominal/$channel ] ; then
        ls -1 /mnt/hadoop/store/group/local/cmstop/jthiema/2016_old/spinCorrInputSymmetrized_2016/Nominal/$channel/${channel}_$sample*.root >> TUnfoldFileLists/TUnfoldFileList_Nominal\_$channel.txt
        ls -1 /mnt/hadoop/store/group/local/cmstop/jthiema/2016_old/spinCorrInputSymmetrized_2016/Nominal/$channel/${channel}_$sample*.root >> TUnfoldFileLists/TUnfoldFileList_Nominal\_combined.txt
    fi
  end
