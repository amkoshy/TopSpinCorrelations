...
public:
    ...
    //ajeeta
    void setOutputFile(const TString& outputFilename,
                       const Channel::Channel& channel, const Systematic::Systematic& systematic);

    /// Write trees with MVA input variables in own file
    //ajeeta -- lets you save weightedEvents histogram with the trees
    void writeTrees(TH1* h_weightedEvents); 
...
protected:
    ...
    /// Create single branch for TTree based on MvaInputVariables of type Float_t
    void createBranch(TTree* tree, const VariableFloatVector& variable)const;
